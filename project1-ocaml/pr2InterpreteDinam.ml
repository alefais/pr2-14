...
(*Le modifiche da apportare all'interprete del linguaggio nel caso
 si voglia assumere una regola di scoping dinamico riguardano la 
 parte relativa alle funzioni: non sarà più necessario utilizzare 
 una chiusura, salvando l'ambiente presente al momento della 
 dichiarazione, ma basterà utilizzare l'ambiente presente al 
 momento dell'applicazione della funzione*)
...
type dval =
	...
	| Dbool of bool
	| Dintlist of intlist
	| Dfun of ide * expr 	(*non è più necessario l'ambiente dval env*)
	| Unbound
...
let dvaltoeval (d:dval) =
	...
	| Dfun(_, _) -> failwith("Not an expressible value")
	| Unbound -> Unknown
...
let rec sem ((e:expr), (env:dval env)) =
	...
	| Applyfun(e1, e2) -> 
		...
		| Dfun(param, body) ->	(*si utilizza l'ambiene env presente al momento dell'applicazione della funzione*)
			let env2 = bind(env, param, evaltodval(sem(e2, env)))
			in sem(body, env2) 
		...
		| Dfun(param, body) -> 	(*si utilizza l'ambiene env presente al momento dell'applicazione della funzione*)
			...
			| Eintlist(l) ->
				let rec mapl (lis:intlist) =
					...
					| Nil -> Nil
					| Cons(x, xs) ->
						let env2 = bind(env, param, Dint(x))
